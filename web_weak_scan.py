        ...
        for url in url_list:
            ...
            try:
                ...
                if "Please enable JavaScript" in response.text:
                    ...
                    result.append(str(self.no) + " " + bcolors.WARNING + url + ": 수동점검필요 - Enable JS" + bcolors.ENDC)
                ...
                if "리다이렉션 되는 URL 넣어주세요" in response.url:
                    ...
                    result.append(str(self.no) + " " + bcolors.FAIL + url + ": 취약 - 페이지 노출(리다이렉션)" + bcolors.ENDC)
                    # warningSite.append = no
                    self.append_Data(url, "취약", "페이지 노출(리다이렉션)")
                else:
                    print(str(self.no) + " " + bcolors.OKGREEN + url + ": 양호(해당 없음) - 페이지 노출(리다이렉션)" + bcolors.ENDC)
                    result.append(str(self.no) + " " + bcolors.OKGREEN + url + ": 양호(해당 없음) - 페이지 노출(리다이렉션)" + bcolors.ENDC)
                ...
                if "보안정책에 따라 차단" in response.text:
                    ...
                    result.append(str(self.no) + " " + bcolors.OKGREEN + url + ": 양호 - 보안정책 페이지" + bcolors.ENDC)
                ...
                if "invalid page fault" in response.text:
                    ...
                    result.append(str(self.no) + " " + bcolors.OKGREEN + url + ": 양호 - 기타 웹 에러 페이지" + bcolors.ENDC)
                ...
                if "error page test" in response.text:
                    ...
                    result.append(str(self.no) + " " + bcolors.OKGREEN + url + ": 양호 - 별도의 접근제어 페이지(error page test)" + bcolors.ENDC)
                ...
                if "Error 404" in response.text:
                    ...
                    result.append(str(self.no) + " " + bcolors.FAIL + url + ": 취약 - 페이지 노출(WEB 서버 404 페이지)" + bcolors.ENDC)
                    ...
                    result.append(str(self.no) + " " + bcolors.OKGREEN + url + ": 양호(해당 없음) - 페이지 노출(WEB 서버 404 페이지)" + bcolors.ENDC)
                ...
                if "Wrong approach path" in response.text:
                    ...
                    result.append(str(self.no) + " " + bcolors.OKGREEN + url + ": 양호 - 별도의 접근제어 페이지(Wrong approach path)" + bcolors.ENDC)
                ...
                if "/error/error_img" in response.text:
                    ...
                    result.append(str(self.no) + " " + bcolors.OKGREEN + url + ": 양호 - 별도의 접근제어 페이지(/error/error_img)" + bcolors.ENDC)
                ...
                if "Error 403</h2>" in response.text:
                    ...
                    result.append(str(self.no) + " " + bcolors.FAIL + url + ": 취약 - 페이지 노출(IIS 403 페이지)" + bcolors.ENDC)
                    self.append_Data(url, "취약", "페이지 노출(IIS 403 페이지)")
                else:
                    print(str(self.no) + " " + bcolors.OKGREEN + url + ": 양호(해당 없음) - 페이지 노출(IIS 403 페이지)" + bcolors.ENDC)
                    result.append(str(self.no) + " " + bcolors.OKGREEN + url + ": 양호(해당 없음) - 페이지 노출(IIS 403 페이지)" + bcolors.ENDC)
                ...
                if "You are unauthorized to access this page." in response.text:
                    ...
                    result.append(str(self.no) + " " + bcolors.OKGREEN + url + ": 양호 - WAS 403 접근제어 페이지" + bcolors.ENDC)
                ...
                if "you've successfully installed Tomcat" in response.text:
                    ...
                    result.append(str(self.no) + " " + bcolors.FAIL + url + ": 취약 - 페이지 노출(Apache 기본 페이지)" + bcolors.ENDC)
                    # warningSite.append = no
                    ...
                    result.append(str(self.no) + " " + bcolors.OKGREEN + url + ": 양호(해당 없음) - 페이지 노출(Apache 기본 페이지)" + bcolors.ENDC)
                ...
                if "IIS Windows Server" in response.text:
                    ...
                    result.append(str(self.no) + " " + bcolors.FAIL + url + ": 취약 - 페이지 노출(IIS 기본 페이지)" + bcolors.ENDC)
                    self.append_Data(url, "취약", "페이지 노출(IIS 기본 페이지)")
                else:
                    print(str(self.no) + " " + bcolors.OKGREEN + url + ": 양호(해당 없음) - 페이지 노출(IIS 기본 페이지)" + bcolors.ENDC)
                    result.append(str(self.no) + " " + bcolors.OKGREEN + url + ": 양호(해당 없음) - 페이지 노출(IIS 기본 페이지)" + bcolors.ENDC)
                ...
                if "아이디" in response.text or "password" in response.text or "login" in response.text:
                    if "https" in response.url:
                        ...
                        result.append(str(self.no) + " " + bcolors.FAIL + url + ": 취약 - 페이지 노출" + bcolors.ENDC)
                        self.append_Data(url, "취약", "페이지 노출")
                    else:
                        print(str(self.no) + " " + bcolors.FAIL + url + ": 취약 - 페이지 노출(로그인 http)" + bcolors.ENDC)
                        result.append(str(self.no) + " " + bcolors.FAIL + url + ": 취약 - 페이지 노출(로그인 http)" + bcolors.ENDC)
                ...
                result.append(str(self.no) + " " + bcolors.OKCYAN + url + ": 양호 - 예외처리(응답없음)" + bcolors.ENDC)
